<?php 
	...
	if(isset($_POST['pseudo'])){
		...
		$req = $bdd->prepare('SELECT * FROM secretaire WHERE pseudo = :pseudo');
		$req->execute(
			array(
				'pseudo' => $pseudo
			)
		...
		else
		{
			...
			else
			{
				...
				$_SESSION = array();
				$_SESSION['utilisateur_id'] = $utilisateur['S_id'];
				$_SESSION['utilisateur_pseudo'] = $utilisateur['pseudo'];
				$_SESSION['secretaire'] = 'true';

				// Rediriger l'utilisateur vers la page d'accueil
				header('Location: pagedeconnexionSecretaire.php');
